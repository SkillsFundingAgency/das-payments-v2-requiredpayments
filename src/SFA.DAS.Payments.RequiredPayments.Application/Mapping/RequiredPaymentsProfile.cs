    ...
    public class RequiredPaymentsProfile : Profile
    {
        public RequiredPaymentsProfile()
        {
            ...
            CreateMap<PaymentModel, CalculatedRequiredLevyAmount>()
                //These 5 Fields are not available in PaymentModel and therefore will not be written to DB so no need to map them, also SequenceNumber
                ...
                .ForMember(dest => dest.CollectionPeriod, opt => opt.MapFrom(src => src.CollectionPeriod.Clone()))
                ...
                .ForMember(dest => dest.DeliveryPeriod, opt => opt.MapFrom(src => src.DeliveryPeriod))
                ...
                .ForMember(dest => dest.LearningAim, opt => opt.MapFrom(s => new LearningAim
                {
            ...
            CreateMap<PayableEarningEvent, CalculatedRequiredOnProgrammeAmount>()
                ...
                .ForMember(x => x.FundingPlatformType, opt => opt.MapFrom(source => source.FundingPlatformType))
                //pull start date from price episode if available (in the case of on prog earnings) or from the aim if not (in the case of functional skills)
                .ForMember(requiredPayment => requiredPayment.LearningStartDate,
                    opt => opt.MapFrom((earning, destination) =>
                        earning.PriceEpisodes
            ...
            CreateMap<IdentifiedRemovedLearningAim, PeriodisedRequiredPaymentEvent>()
                ...
                .ForPath(x => x.FundingPlatformType, opt => opt.MapFrom(src => FundingPlatformType.SubmitLearnerData))
